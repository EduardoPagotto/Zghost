...
public:
    ...
    inline void add(const uint8_t &val) { std::tie(*this->high, *this->low) = R16::splitword(this->get() + val); }

    inline static std::tuple<uint8_t, uint8_t> splitword(uint16_t w) {
        return std::make_tuple(w >> 8, w & 0xff);
    }

    inline static uint16_t joinBytes(const uint8_t &h, const uint8_t &l) {
        return  static_cast<uint16_t>((h << 8) | l);
    }

    // static int16_t signExtend(const uint8_t &v) {
