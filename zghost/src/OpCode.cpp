...
void instr__EX_iSP_HL(Z80* z, const uint8_t& opcode) {
    z->Tstates += 19;
    uint16_t bytetempl = z->readMem(z->sp);
    uint16_t bytetemph = z->readMem(z->sp + 1);
    // z->Memory.ContendReadNoMreq(z->SP()+1, 1)
    z->writeMem(z->sp + 1, z->H);
    z->writeMem(z->sp, z->L);
